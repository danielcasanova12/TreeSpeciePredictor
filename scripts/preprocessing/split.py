...
Exemplos:
  ...
  # Criar apenas o dataset 'tangential' (sem dureza)
  python split.py --tipo tangential --train 0.7 --test 0.2 --val 0.1

  # Criar 'tangential_hardness' (COM tarja de dureza)
  python split.py --tipo tangential_hardness

  # Criar o dataset combinado 'tangential_transverse'
  python split.py --tipo tangential_transverse

  # Criar todos os 6 datasets
  python split.py --tipo all
        """
    )

    parser.add_argument('--tipo', required=True,
                        choices=[
                            'tangential', 'transverse', 'ambos',
                            'tangential_hardness', 'transverse_hardness',
                            'tangential_transverse', 'tangential_transverse_hardness', 
                            'all'
                        ],
                        help='Tipo de dataset a criar')
    parser.add_argument('--train', type=float, default=0.7, help='Proporção treino (padrão: 0.7)')
    parser.add_argument('--test', type=float, default=0.2, help='Proporção teste (padrão: 0.2)')
    parser.add_argument('--val', type=float, default=0.1, help='Proporção validação (padrão: 0.1)')
    parser.add_argument('--seed', type=int, default=42, help='Seed (padrão: 42)')
    parser.add_argument('--altura-barra', type=int, default=30, help='Altura da barra em pixels (padrão: 30)')
    parser.add_argument('--pasta-dados', type=str, default='../../data', help='Pasta base dos dados (padrão: ../../data)')
    parser.add_argument('--max-por-classe', type=int, default=120, help='TOTAL por classe (somando train/test/val) (padrão: 120)')
    ...
    datasets = {
        'tang': base_dir / 'tangential',
        'trans': base_dir / 'transverse',
        'tang_dur': base_dir / 'tangential_hardness',
        'trans_dur': base_dir / 'transverse_hardness',
        'complete': base_dir / 'tangential_transverse',
        'complete_dur': base_dir / 'tangential_transverse_hardness',
    }

    try:
        if args.tipo == 'tangential':
            create_dataset(tang_src, datasets['tang'], args.train, args.test, args.val, args.seed, add_bar=False, bar_height=args.altura_barra, max_per_class=args.max_por_classe)
        
        elif args.tipo == 'transverse':
            create_dataset(trans_src, datasets['trans'], args.train, args.test, args.val, args.seed, add_bar=False, bar_height=args.altura_barra, max_per_class=args.max_por_classe)

        elif args.tipo == 'tangential_hardness':
            create_dataset(tang_src, datasets['tang_dur'], args.train, args.test, args.val, args.seed, add_bar=True, bar_height=args.altura_barra, max_per_class=args.max_por_classe)

        elif args.tipo == 'transverse_hardness':
            create_dataset(trans_src, datasets['trans_dur'], args.train, args.test, args.val, args.seed, add_bar=True, bar_height=args.altura_barra, max_per_class=args.max_por_classe)

        elif args.tipo == 'ambos':
            create_dataset(tang_src, datasets['tang'], args.train, args.test, args.val, args.seed, add_bar=False, bar_height=args.altura_barra, max_per_class=args.max_por_classe)
            create_dataset(trans_src, datasets['trans'], args.train, args.test, args.val, args.seed, add_bar=False, bar_height=args.altura_barra, max_per_class=args.max_por_classe)

        elif args.tipo == 'tangential_transverse':
            if not datasets['tang'].exists() or not datasets['trans'].exists():
                raise FileNotFoundError("'tangential' ou 'transverse' não encontrados. Rode '--tipo ambos' primeiro.")
            create_paired_composite_dataset(datasets['tang'], datasets['trans'], datasets['complete'], add_hardness_bar=False, bar_height=args.altura_barra)

        elif args.tipo == 'tangential_transverse_hardness':
            if not datasets['tang'].exists() or not datasets['trans'].exists():
                raise FileNotFoundError("'tangential' ou 'transverse' não encontrados. Rode '--tipo ambos' primeiro.")
            create_paired_composite_dataset(datasets['tang'], datasets['trans'], datasets['complete_dur'], add_hardness_bar=True, bar_height=args.altura_barra)

        elif args.tipo == 'all':
            print(f"\n{'='*60}\nMODO ALL - 6 datasets\n{'='*60}\n")
            steps = [
                ("1/6", "Tangencial (sem dureza)", tang_src, datasets['tang'], False),
                ("2/6", "Transversal (sem dureza)", trans_src, datasets['trans'], False),
                ("3/6", "Tangencial + dureza", tang_src, datasets['tang_dur'], True),
                ("4/6", "Transversal + dureza", trans_src, datasets['trans_dur'], True),
            ]
            for step, name, src, dst, bar in steps:
                print(f"\n🔹 [{step}] {name}")
                create_dataset(src, dst, args.train, args.test, args.val, args.seed, add_bar=bar, bar_height=args.altura_barra, max_per_class=args.max_por_classe)

            print(f"\n🔹 [5/6] tangential_transverse (pareado SEM dureza)")
            create_paired_composite_dataset(datasets['tang'], datasets['trans'], datasets['complete'], add_hardness_bar=False, bar_height=args.altura_barra)

            print(f"\n🔹 [6/6] tangential_transverse_hardness (pareado COM tarja)")
            create_paired_composite_dataset(datasets['tang'], datasets['trans'], datasets['complete_dur'], add_hardness_bar=True, bar_height=args.altura_barra)
            
            print(f"\n{'#'*60}\n# ✅ 6 DATASETS CRIADOS COM SUCESSO!\n{'#'*60}\n")
